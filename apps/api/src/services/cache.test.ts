...
describe('cache', () => {
  ...
  describe('createKVCache', () => {
    it('stores small values as plain JSON strings', async () => {
      ...
      const putCalls = (kv.put as unknown as ReturnType<typeof vi.fn>).mock
        .calls
  ...
  describe('CacheKeys', () => {
    ...
    it('generates correct events key with explicit time bounds', () => {
      expect(
        CacheKeys.events('ABC123', 5, 'private', 'uid-1', 1000, 2000),
      ).toBe(
    ...
    it('generates correct augmented events key', () => {
      expect(
        ...
        'augmented:v18:ABC123:5:v1.2.0:inferThreatReduction:true:visibility:private:scope:uid:uid-1',
      )
    })
