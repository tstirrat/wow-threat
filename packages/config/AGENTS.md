...
4. Bump owning top-level numeric config `version`
5. Run scoped checks:
...
- Any effective config change must bump the owning top-level `version` in:
  - `packages/config/src/era/index.ts`
  - `packages/config/src/sod/index.ts`
  - `packages/config/src/tbc/index.ts`
- `packages/config/src/version.ts` concatenates the three top-level versions to
  derive `configCacheVersion` automatically for events cache-busting.
