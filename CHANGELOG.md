...
- **web**: Integrate PostHog product analytics (`posthog-js`). Tracks `$pageview` on every route change and key user-flow events: `search_used` (report URL submitted), `report_loaded` (report_id, fight_count, duration_ms), `fight_selected` (report_id, fight_id, boss_name), and `threat_chart_viewed` (report_id, fight_id, player_count). Autocapture, session recording, and anonymous person profiles are disabled. PostHog is a no-op when `VITE_POSTHOG_KEY` is unset. CSP `connect-src` updated to include PostHog ingestion endpoints. Board must provision a PostHog project key in the production environment to activate analytics ([AGE-13]).
